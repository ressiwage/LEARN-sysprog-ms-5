...
int main(int argc, char **argv)
{
	...
	while ((argind = getopt(argc, argv, "abc:")) != -1)
	{
		optind++;
	}
	int num_files = argc - optind;
	int **output, **p_arrays;
	int **tmp = malloc(sizeof(int *) * num_files + sizeof(int) * MAX_ELEMENTS_IN_FILE * num_files); // указатели на массивы с числами
	if (tmp != NULL)
	{
		p_arrays = tmp;
	}
	...
	{
		printf("err alloc p_arr");
		exit(-1);
	}
tmp = malloc(sizeof(int *) * num_files + sizeof(int) * MAX_ELEMENTS_IN_FILE * num_files);
	if (tmp != NULL)
	{
		output = tmp;
	}
